...
func SortQuadruples(i []int, x, y, z []float64, by string) (I []int, X, Y, Z []float64) {
    ...
    switch by {
    case "i": sort.Sort(QuadruplesByI{q})
    case "x": sort.Sort(QuadruplesByX{q})
    case "y": sort.Sort(QuadruplesByY{q})
    case "z": sort.Sort(QuadruplesByZ{q})
    default: Panic(_sorting_err3, by)
    }
    if i != nil {
        I = q.I()
    }
