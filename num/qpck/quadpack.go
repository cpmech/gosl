...
void dqagse_(fType f, double* a, double* b, double* epsabs, double* epsrel, int* limit,
             ...
             double* rlist, double* elist, int* iord, int* last, int* fid);
...
func Qagse(fid int, f fType, a, b, epsabs, epsrel float64, alist, blist, rlist, elist []float64, iord []int32) (result, abserr float64, neval, last int32, err error) {
	...
	if ier != 0 {
		switch ier {
		case 1:
			err = chk.Err("error # 1: maximum number of subdivisions allowed\n")
		case 2:
			err = chk.Err("error # 2: the occurrence of roundoff error is detected\n")
		case 3:
			err = chk.Err("error # 3: extremely bad integrand behaviour\n")
		case 4:
			err = chk.Err("error # 4: the algorithm does not converge\n")
		case 5:
			err = chk.Err("error # 5: the integral is probably divergent, or slowly convergent\n")
		case 6:
			err = chk.Err("error # 6: the input is invalid\n")
		default:
			err = chk.Err("unknown error\n")
		}
	}
	return
}
