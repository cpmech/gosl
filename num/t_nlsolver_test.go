...
func Test_nls02(tst *testing.T) {
    ...
    Jfcn := func(dfdx *la.Triplet, x []float64) error {
        dfdx.Start()
        dfdx.Put(0, 0, 2.0 + math.Exp(-x[0]));  dfdx.Put(0, 1, -1.0)
        dfdx.Put(1, 0, -1.0);                   dfdx.Put(1, 1, 2.0 + math.Exp(-x[1]))
        return nil
    }
...
func Test_nls03(tst *testing.T) {
    ...
    Jfcn := func(dfdx *la.Triplet, x []float64) error {
        dfdx.Start()
        dfdx.Put(0, 0, 0.5*x[1]*cos(x[0]*x[1])-0.5);              dfdx.Put(0, 1, 0.5*x[0]*cos(x[0]*x[1])-0.25/pi)
        dfdx.Put(1, 0, (2.0-0.5/pi)*math.Exp(2.0*x[0]) - 2.0*e);  dfdx.Put(1, 1, e/pi)
        return nil
    }
    JfcnD := func(dfdx [][]float64, x []float64) error {
        dfdx[0][0] = 0.5*x[1]*cos(x[0]*x[1])-0.5;              dfdx[0][1] = 0.5*x[0]*cos(x[0]*x[1])-0.25/pi
        dfdx[1][0] = (2.0-0.5/pi)*math.Exp(2.0*x[0]) - 2.0*e;  dfdx[1][1] = e/pi
        return nil
    }
