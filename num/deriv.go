...
func DerivCen5(f Cb_fx, x, h float64) (res, abserr float64) {
	...
	if round < trunc && (round > 0 && trunc > 0) {
		...
		r_opt, round_opt, trunc_opt := centralDeriv5(f, x, h_opt)
		error_opt := round_opt + trunc_opt

		/* Check that the new error is smaller, and that the new derivative
		   is consistent with the error bounds of the original estimate. */

		if error_opt < err && math.Abs(r_opt-res) < 4.0*err {
			res = r_opt
			err = error_opt
		}
	}
...
func DerivFwd4(f Cb_fx, x, h float64) (res, abserr float64) {
	...
	if round < trunc && (round > 0 && trunc > 0) {
		...
		r_opt, round_opt, trunc_opt := forwardDeriv4(f, x, h_opt)
		error_opt := round_opt + trunc_opt
...
func DerivBackward(f Cb_fx, x, h float64) (res, abserr float64) {
