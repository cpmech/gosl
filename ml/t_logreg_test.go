...
func TestLogReg00(tst *testing.T) {
	...
	for _, θ0 := range []float64{0.5, 1.0} {
		for _, θ1 := range []float64{0.5, 1.0} {
			...
			d.params[1] = θ1
			r.Deriv(dCdθ, d)
			chk.DerivScaVec(tst, "dCdθ", 1e-7, dCdθ, d.params, 1e-6, chk.Verbose, func(th []float64) float64 {
				copy(d.params, th)
				return r.Cost(d)
			})
		}
...
func TestLogReg01(tst *testing.T) {
	...
	chk.Array(tst, "θ", 1e-14, d.params, []float64{-2.400009669708430e+01, 1.957478716620902e-01, 1.933175159514175e-01})
	...
	useZeroGuess := true
	if useZeroGuess {
		d.params.Fill(0) // start with challenging initial θ
	} else {
		d.params.Apply(1, θ)
	}
	io.Pl()
	...
	chk.Array(tst, "θ", 1e-14, d.params, []float64{-2.516133256589910e+01, 2.062317052577260e-01, 2.014715922708144e-01})
...
func TestLogReg02(tst *testing.T) {
	...
	for _, θ0 := range []float64{0.5, 1.0} {
		for _, θ1 := range []float64{0.5, 1.0} {
			...
			d.params[1] = θ1
			r.Deriv(dCdθ, d)
			chk.DerivScaVec(tst, "dCdθ", 1e-7, dCdθ, d.params, 1e-6, verb, func(th []float64) float64 {
				copy(d.params, th)
				return r.Cost(d)
			})
		}
	...
	chk.Array(tst, "θ[:5]", 1e-14, d.params[:5], []float64{6.527575848138054e-01, -1.730594903181217e-01, 3.615618466861891e-01, -1.194645899263627e+00, -4.186288373383852e-01})
	...
	chk.Array(tst, "θ[:5]", 1e-14, d.params[:5], []float64{1.272656700281225, 6.252526148274546e-01, 1.180976145721166, -2.019842398401904, -9.173659359499787e-01})
