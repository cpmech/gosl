...
func TestSparseLA01(tst *testing.T) {
	...
	utl.CheckVector(tst, "x += a * (2*u + 3*W)", 1e-17, x, []float64{1176, 2017.6, 4760})
...
func TestSparseLA02(tst *testing.T) {
	...
	utl.CheckVectorC(tst, "p = a*u", 1e-17, p, []complex128{5.5, 0.55, 55})

	utl.Pf("\nfunc SpMatVecMulAdd(v []float64, α float64, a [][]float64, u []float64)\n")
	SpMatVecMulAddC(s, 1, a, u) // s += dot(a, u)
	PrintVecC("s += a*u", s, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "s += a*u", 1e-17, s, []complex128{1005.5, 1000.55, 1055})

	utl.Pf("\nfunc SpMatTrVecMul(v []float64, α float64, a [][]float64, u []float64)\n")
	q := make([]complex128, 5)
	SpMatTrVecMulC(q, 1, a, w) // q = dot(transpose(a), w)
	PrintVecC("q = trans(a)*w", q, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "q = trans(a)*w", 1e-17, q, []complex128{312, 624, 936, 1248, 1560})

	utl.Pf("\nfunc SpMatTrVecMulAdd(v []float64, α float64, a [][]float64, u []float64)\n")
	SpMatTrVecMulAddC(r, 1, a, w) // r += dot(transpose(a), w)
	PrintVecC("r += trans(a)*w", r, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "r += trans(a)*w", 1e-17, r, []complex128{1312, 1624, 1936, 2248, 2560})
}
...
func TestSparseLA03(tst *testing.T) {
	...
	SpMatVecMulC(p, 1, a, u) // p := 1*a*u
	PrintVecC("p = a*u", p, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "p = a*u", 1e-17, p, []complex128{6.5 + 34i, 0.55 + 2.2i, 38 + 320i})

	utl.Pf("\nfunc SpMatVecMulAdd(v []float64, α float64, a [][]float64, u []float64)\n")
	SpMatVecMulAddC(s, 1, a, u) // s += dot(a, u)
	PrintVecC("s += a*u", s, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "s += a*u", 1e-15, s, []complex128{1006.5 + 33i, 1000.55 + 1.2i, 1038 + 319i})

	utl.Pf("\nfunc SpMatTrVecMul(v []float64, α float64, a [][]float64, u []float64)\n")
	q := make([]complex128, 5)
	SpMatTrVecMulC(q, 1, a, w) // q = dot(transpose(a), w)
	PrintVecC("q = trans(a)*w", q, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "q = trans(a)*w", 1e-14, q, []complex128{312.5 + 20.95i, 625 + 51.9i, 935 - 37.15i, 1245 + 3.8i, 1557.5 + 4.75i})

	utl.Pf("\nfunc SpMatTrVecMulAdd(v []float64, α float64, a [][]float64, u []float64)\n")
	SpMatTrVecMulAddC(r, 1, a, w) // r += dot(transpose(a), w)
	PrintVecC("r += trans(a)*w", r, "(%2g", " +%4gi)  ", false)
	utl.CheckVectorC(tst, "r += trans(a)*w", 1e-14, r, []complex128{1312.5 + 21.95i, 1625 + 52.9i, 1935 - 36.15i, 2245 + 4.8i, 2557.5 + 5.75i})
}
...
func TestSparseLA04(tst *testing.T) {
	...
	utl.CompareDbls(tst, "a.x", a.x, []float64{1, 2, 3, 4})
	utl.CompareInts(tst, "a.i", a.i, []int{0, 1, 2, 2})
	utl.CompareInts(tst, "a.p", a.p, []int{0, 2, 3, 4})
	...
	utl.CompareDbls(tst, "b.x", b.x, []float64{3, 2, 1, 1})
	utl.CompareInts(tst, "b.i", b.i, []int{1, 0, 1, 2})
	utl.CompareInts(tst, "b.p", b.p, []int{0, 1, 2, 4})

	c, a2c, b2c := SpAllocMatAddMat(a, b)
	SpMatAddMat(c, 1, a, 1, b, a2c, b2c)
	utl.Pf("c = %+v\n", c)
	PrintMat("c", c.ToDense(), "%2g ", false)
	utl.CompareDbls(tst, "c.x", c.x, []float64{1, 5, 2, 3, 1, 5})
	utl.CompareInts(tst, "c.i", c.i, []int{0, 1, 0, 2, 1, 2})
	utl.CompareInts(tst, "c.p", c.p, []int{0, 2, 4, 6})
	utl.CompareInts(tst, "a2c", a2c, []int{0, 1, 3, 5})
	utl.CompareInts(tst, "b2c", b2c, []int{1, 2, 4, 5})
	utl.CheckMatrix(tst, "c", 1e-17, c.ToDense(), [][]float64{{1, 2, 0}, {5, 0, 1}, {0, 3, 5}})
}
...
func TestSparseLA05(tst *testing.T) {
	...
	utl.Pf("a = %+v\n", a)
	PrintMat("a", a.ToDense(), "%2g ", false)
	utl.CompareDbls(tst, "a.x", a.x, []float64{1, 2, 3, 3, 1, 1, 5, 7, 8})
	utl.CompareInts(tst, "a.i", a.i, []int{0, 2, 4, 1, 3, 0, 4, 0, 2})
	utl.CompareInts(tst, "a.p", a.p, []int{0, 3, 3, 5, 7, 7, 9})
	...
	utl.Pf("b = %+v\n", b)
	PrintMat("b", b.ToDense(), "%2g ", false)
	utl.CompareDbls(tst, "b.x", b.x, []float64{1, 8, 1, 2, 3, 4, 5, 5, 1, 4, 2, 1})
	utl.CompareInts(tst, "b.i", b.i, []int{1, 3, 0, 1, 2, 3, 4, 2, 0, 1, 2, 4})
	utl.CompareInts(tst, "b.p", b.p, []int{0, 0, 2, 7, 7, 8, 12})

	c, a2c, b2c := SpAllocMatAddMat(a, b)
	SpMatAddMat(c, 0.1, a, 2, b, a2c, b2c)
	utl.Pf("c = %+v\n", c)
	PrintMat("c", c.ToDense(), "%10.6f ", false)
	utl.CompareDbls(tst, "c.x", c.x, []float64{0.1, 0.2, 0.3, 2, 16, 2, 4.3, 6, 8.1, 10, 0.1, 0.5, 10, 2.7, 8, 4.8, 2})
	utl.CompareInts(tst, "c.i", c.i, []int{0, 2, 4, 1, 3, 0, 1, 2, 3, 4, 0, 4, 2, 0, 1, 2, 4})
	utl.CompareInts(tst, "c.p", c.p, []int{0, 3, 5, 10, 12, 13, 17})
	utl.CompareInts(tst, "a2c", a2c, []int{0, 1, 2, 6, 8, 10, 11, 13, 15})
	utl.CompareInts(tst, "b2c", b2c, []int{3, 4, 5, 6, 7, 8, 9, 12, 13, 14, 15, 16})
	utl.CheckMatrix(tst, "c", 1e-16, c.ToDense(), [][]float64{
		{0.1, 0, 2.0, 0.1, 0, 2.7},
		{0, 2, 4.3, 0, 0, 8.0},
		{0.2, 0, 6.0, 0, 10, 4.8},
		{0, 16, 8.1, 0, 0, 0.0},
...
func TestSparseLA06(tst *testing.T) {
	...
	utl.CompareInts(tst, "c.i", c.i, []int{0, 2, 4, 1, 3, 0, 1, 2, 3, 4, 0, 4, 2, 0, 1, 2, 4})
	utl.CompareInts(tst, "c.p", c.p, []int{0, 3, 5, 10, 12, 13, 17})
	utl.CompareInts(tst, "d.i", d.i, []int{0, 2, 4, 1, 3, 0, 1, 2, 3, 4, 0, 4, 2, 0, 1, 2, 4})
	utl.CompareInts(tst, "d.p", d.p, []int{0, 3, 5, 10, 12, 13, 17})
	...
	utl.CompareDbls(tst, "c.x", c.x, []float64{0.1, 0.2, 0.3, 2, 16, 2, 4.3, 6, 8.1, 10, 0.1, 0.5, 10, 2.7, 8, 4.8, 2})
	utl.CompareDbls(tst, "d.x", d.x, []float64{0.1, 0.2, 0.3, 2, 16, 2, 4.3, 6, 8.1, 10, 0.1, 0.5, 10, 2.7, 8, 4.8, 2})
	utl.CompareDbls(tst, "d.z", d.z, []float64{1, 2, 3, 0, 0, 0, 3, 0, 1, 0, 1, 5, 0, 7, 0, 8, 0})
	utl.CompareInts(tst, "a2c", a2c, []int{0, 1, 2, 6, 8, 10, 11, 13, 15})
	utl.CompareInts(tst, "b2c", b2c, []int{3, 4, 5, 6, 7, 8, 9, 12, 13, 14, 15, 16})
	utl.CheckMatrix(tst, "c", 1e-16, c.ToDense(), [][]float64{
		{0.1, 0, 2.0, 0.1, 0, 2.7},
	...
	utl.CheckMatrixC(tst, "d", 1e-16, d.ToDense(), [][]complex128{
...
func TestSparseLA07(tst *testing.T) {
	...
	utl.Pf("tc = %+v\n", tc)
	utl.CheckMatrix(tst, "c", 1e-16, tc.ToMatrix(nil).ToDense(), [][]float64{
		{0.1, 0, 2.0, 0.1, 0, 2.7},
		{0, 2, 4.3, 0, 0, 8.0},
		{0.2, 0, 6.0, 0, 10, 4.8},
		{0, 16, 8.1, 0, 0, 0.0},
...
func TestSparseLA08(tst *testing.T) {
	...
	utl.Pf("td = %+v\n", td)
	utl.CheckMatrixC(tst, "d", 1e-16, td.ToMatrix(nil).ToDense(), [][]complex128{
		{0.1 + 1i, 0, 2.0, 0.1 + 1i, 0, 2.7 + 7i},
		{0, 2, 4.3 + 3i, 0, 0, 8.0},
		{0.2 + 2i, 0, 6.0, 0, 10, 4.8 + 8i},
		{0, 16, 8.1 + 1i, 0, 0, 0.0},
...
func TestSparseLA10(tst *testing.T) {
	...
	//utl.Tsilent = false
	utl.TTitle("TestSparse LA10: SpTriMatTrVecMul")
	...
	utl.CheckVector(tst, "y=Tr(a)*x", 1e-17, y, []float64{8.4, 18.6, 25.2, 37.2, 42})
}
