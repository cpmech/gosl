...
func Test_delaunay01(tst *testing.T) {
	...
	chk.Ints(tst, "verts of cell 0", M.Cells[0].V, []int{3, 0, 4})
	chk.Ints(tst, "verts of cell 1", M.Cells[1].V, []int{4, 1, 2})
	chk.Ints(tst, "verts of cell 2", M.Cells[2].V, []int{1, 4, 0})
	chk.Ints(tst, "verts of cell 3", M.Cells[3].V, []int{4, 2, 3})

	if chk.Verbose {
		plt.SetForEps(1, 300)
		M.Draw2d(true, false, nil, &plt.Fmt{C: "blue", M: "*"})
