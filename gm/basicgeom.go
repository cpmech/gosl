...
func PointsLims(pp []*Point) (cmin, cmax []float64) {
    ...
    for i := 1; i < len(pp); i++ {
        if pp[i].X < cmin[0] { cmin[0] = pp[i].X }
        if pp[i].Y < cmin[1] { cmin[1] = pp[i].Y }
        if pp[i].Z < cmin[2] { cmin[2] = pp[i].Z }
        if pp[i].X > cmax[0] { cmax[0] = pp[i].X }
        if pp[i].Y > cmax[1] { cmax[1] = pp[i].Y }
        if pp[i].Z > cmax[2] { cmax[2] = pp[i].Z }
    }
    return
}
...
func IsPointIn(p *Point, cMin, cMax []float64, tol float64) bool {
    if p.X < cMin[0] - tol || p.X > cMax[0] + tol { return false }
    if p.Y < cMin[1] - tol || p.Y > cMax[1] + tol { return false }
    if p.Z < cMin[2] - tol || p.Z > cMax[2] + tol { return false }
    return true
}
