...
// Enodes returns the indices of control points (nodes) for each element
func (o *Nurbs) Enodes() (enodes [][]int) {
    switch o.gnd {
    // curve
    case 1:
        s0 := o.b[0].Elements()
        m0 := o.p[0]+1 // number of basis functions
        enodes = make([][]int, len(s0))
        for ι, su := range s0 {
            c := 0
            enodes[ι] = make([]int, m0)
            for r := 0; r < m0; r++ {
                enodes[ι][c] = su[0] - o.p[0] + r
                c += 1
            }
        }
    // surface
    case 2:
        s0, s1 := o.b[0].Elements(), o.b[1].Elements()
        m0, m1 := o.p[0]+1, o.p[1]+1 // number of basis functions
        enodes  = make([][]int, len(s0) * len(s1))
        for κ, sv := range s1 {
            for ι, su := range s0 {
                c := 0
                e := ι + κ * len(s0)
                enodes[e] = make([]int, m0 * m1)
                for s := 0; s < m1; s++ {
                    j := sv[0] - o.p[1] + s
                    for r := 0; r < m0; r++ {
                        i := su[0] - o.p[0] + r
                        enodes[e][c] = i + j * o.n[0]
                        c += 1
                    }
                }
            }
    ...
    case 3:
        s0, s1, s2 := o.b[0].Elements(), o.b[1].Elements(), o.b[2].Elements()
        m0, m1, m2 := o.p[0]+1, o.p[1]+1, o.p[2]+1 // number of basis functions
        enodes  = make([][]int, len(s0) * len(s1) * len(s2))
        for λ, sw := range s2 {
            for κ, sv := range s1 {
                for ι, su := range s0 {
                    c := 0
                    e := ι + κ * len(s0) + λ * len(s0) * len(s1)
                    enodes[e] = make([]int, m0 * m1 * m2)
                    for t := 0; t < m2; t++ {
                        k := sw[0] - o.p[2] + t
                        for s := 0; s < m1; s++ {
                            j := sv[0] - o.p[1] + s
                            for r := 0; r < m0; r++ {
                                i := su[0] - o.p[0] + r
                                enodes[e][c] = i + j * o.n[0] + k * o.n[0] * o.n[1]
                                c += 1
                            }
                        }
                    }
...
func krefine(Unew [][]float64, Qnew, Qold [][][][]float64, dir int, X, U []float64, nn, pp []int, a, b int) {
    ...
    qqnew := func(f, g, i int) []float64 {
        if dir == 0 { return Qnew[i][f][g] }
        if dir == 1 { return Qnew[f][i][g] }
        return Qnew[f][g][i]
    }
    qqold := func(f, g, i int) []float64 {
        if dir == 0 { return Qold[i][f][g] }
        if dir == 1 { return Qold[f][i][g] }
        return Qold[f][g][i]
    }
